...
To do this we're going to use Old Woman Creek Estuary as an example. We will be estimating the phosphorus load entering the system. We'll get to retention later, but it's going to be a lot more complicated because there are different frequencies of data at the inflow and outflow of the wetland. <br>
...
As a note: we're using ISO week here so we also need to use iso year in order to make sure that every week is measured separately. The code will get screwy if you don't do this because then you have duplicates because it shoves the wrong samples into the wrong years. Iso calender is a whole separate calendar from our normal one so that's why it gets weird. 
...
An important note here is that we don't want to resample hydrology data, we want to ONLY resample phosphorus data and then APPLY it to the continous (daily) hydrology data. The goal is to understand how nutrient sampling frequency affects estimates of retention.
...
## Plotting the data
Lets take a look at phosphorus concentrations to see if we can explain this: 

```{r}
ggplot(owcBR_GF, aes(x=Date, y=TP)) +
  geom_line() + 
  xlab("") + ylab("TP ug/L")+theme_classic()

ggplot(owcBR_GF, aes(x=Date, y=P_lbs)) +
  geom_line() + 
  xlab("") + ylab("TP load (lbs)")+theme_classic()

ggplot(owcBR_GF, aes(x=Date, y=discharge_L)) +
  geom_line() + 
  xlab("") + ylab("Discharge (L)")+theme_classic()

mid<-26
library(ggpubr)
ggplot(owcBR_GF, aes(x=TP, y=discharge_L, color = week)) +
  geom_point() + 
  xlab("TP (mg/L)") + ylab("Discharge (L)")+ 
  scale_color_gradient2(midpoint=mid, low="mediumspringgreen", mid="slateblue", high="mediumspringgreen")+theme_classic()+
  stat_cor(method = "pearson",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")))+ geom_smooth(method=lm, se=FALSE, fullrange=F) 
```

There is a positive relationship between P and discharge but it's quite a variable relationship.

<a name="stresstests"></a>
...
This is because it's just unlikely to select the max values for every week. I think there's 7^52 (8.812479e+43) possible combinations for each year so it's not unlikely that we wouldn't reach the maximum value. <br>


Now we're going to make some fake data to run the actual algorithm on: 
...
IT'S WORKING!! The same algorithm with less spiky data produces a better estimate when switching from daily to weekly data.
...
```
<br>
A notable differences is the higher spread/abundance of outliers in monthly as compared to weekly sampling. <br>
...
```
A more reasonable number when we scale that up to 100 cfs as our cutoff! 
...
TIME to test a whole bunch of variations in how we're measuring retention! 
...
Things to add: 
Subset 1-4: 1-4 storm events sampled
Subset 5-9: 1-5 storm events sampled @ 25 cfs 
...
DO NOT PUT THIS SECTION FURTHER DOWN. When you add things to the owcBR_GF dataframe you screw up the code so it doesn't work anymore. 

<a name="loadout"></a>

## Lets try out some of the weird methods of gap filling! Flow-weighted means, and linear interpolation
...
We see that the outlet again is underestimated compared to weekly sampling, but much less so than the inlet is! There's also a lot less variability in the range of possible values when changing sampling regime. Interesting finding there.
...
## Now lets try to combine these! Now we can actually measure retention!
...
<br>
Let's highlight this using our Bias and Imprecision values for each method at the inflow vs. outflow. 

```{r}
method <- c('Weekly','Weekly','Monthly','Monthly')
site <- c('Inflow','Outflow','Inflow','Outflow')
bias <- c(-0.405314, -0.1785602, -0.5644752, -0.3110493)
imprecission <- c(0.3728673, 0.2560827, 0.4588433, 0.4045302)
metrics = data.frame(method,site,bias,imprecission)
# plot it out
pbias <- ggplot(data=metrics, aes(x=method, y=bias, fill=site)) +xlab("")+
geom_bar(stat="identity", color="black", position=position_dodge())+
  theme_classic()
pimprec <- ggplot(data=metrics, aes(x=method, y=imprecission, fill=site)) +
geom_bar(stat="identity", color="black", position=position_dodge())+xlab("")+
  theme_classic()
ggarrange(pbias,pimprec)
```
...
ggplot(owcoutflow, aes(x=FlowRate_cms, y=Corrected.Stage_meters)) +
  geom_point() + 
  xlab("Flow Rate (cm/sec)") + ylab("Stage (m)")+
  stat_cor(method = "pearson",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")))+ geom_smooth(method=lm, se=FALSE, fullrange=F) 
...
```
There's not a clear relationship between flow rate and stage. 
<br>
...
```
CLEARLY ILLUSTRATES THE PROBLEM!! This time with REAL DATA!! 
<br> 
Let's calculate biases just to be safe: 
...
```
Lets do some stats with the various methods of calculating bias and imprecision:

```{r}
groupedfigure = subset(grouped, method == "monthly" | method == "monthly out" | method ==  "weekly" | method == "week out" | method == "all_storms" | method == "M100_5" | method == "M25_5" | method == "flow-weighted" | method == "lin interp")


abias = aov(med~method, groupedfigure)
summary(abias)
TukeyHSD(abias)

aimp = aov(imprecision~method, groupedfigure)
summary(aimp)
TukeyHSD(aimp)

# Storms

groupedstorm = subset(grouped, method == "all_storms" | method ==  "M100_1"| method == "M100_2"| method == "M100_3"| method == "M100_4"| method == "M100_5"| method == "M100_10"| method == "M25_1"| method == "M25_2"| method == "M25_3"| method == "M25_4"| method == "M25_5"| method =="M25_10")

asbias = aov(med~method, groupedstorm)
summary(asbias)
TukeyHSD(asbias)

asimp = aov(imprecision~method, groupedstorm)
summary(asimp)
TukeyHSD(asimp)
